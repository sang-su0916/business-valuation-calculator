    ...
    with st.expander("데이터 저장 및 불러오기", expanded=False):
        ...
        with col1:
            ...
            if st.button("현재 입력값 Excel로 다운로드"):
                ...
                st.markdown(get_table_download_link(df, f"{company_name}_평가데이터", "📥 데이터 다운로드"), unsafe_allow_html=True)
            ...
            if uploaded_file is not None:
                try:
                    if uploaded_file.name.endswith(('.xlsx', '.xls')):
                        ...
                        st.warning("현재 Excel(.xlsx, .xls) 파일만 데이터 로드가 지원됩니다.")
